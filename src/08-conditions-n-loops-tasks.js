...
function isInsideCircle(circle, point) {
  const l = Math.sqrt((point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const i = isStartIncluded ? '[' : '(';
  const j = isEndIncluded ? ']' : ')';
  return (a < b) ? `${i}${a}, ${b}${j}` : `${i}${b}, ${a}${j}`;
}
...
function getDigitalRoot(num) {
  ...
  const k = num.toString().split('').map((item) => Number(item)).reduce((s, item) => s + item);
  return getDigitalRoot(k);
}
